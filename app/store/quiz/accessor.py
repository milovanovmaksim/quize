...
class QuizAccessor(BaseAccessor):
    ...
    async def save_questions(self, count: int):
        """
        Отправляет get-запросы на ресурс "https://jservice.io/api/random?count={count}", ответ сохраняет в БД.
        К публичному API с викторинами будут выполняться дополнительные
        запросы до тех пор, пока не будет записано необходимое количество "count" уникальных вопросов в БД.
        Args:
